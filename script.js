document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        showError(field, customMessage || 'This field is required.');
        return false;
      }
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (value.length < min) {
          showError(field, customMessage || 'Minimum ' + min + ' characters required.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('maxlength')) {
        ...
        if (value.length > max) {
          showError(field, customMessage || 'Maximum ' + max + ' characters allowed.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('pattern')) {
        ...
        if (!pattern.test(value)) {
          showError(field, customMessage || 'Please match the required format.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target;
      if (field.matches('input, textarea, select') && field.classList.contains(INVALID_CLASS)) {
        validateField(field);
      }
    });
  ...
  const ANIMATE_SELECTOR = '.animate-on-scroll, [data-animate], .fade-in, .slide-up, .slide-in';
  const ANIMATED_CLASS = 'is-visible';
  const ANIMATION_THRESHOLD = 0.15;

  const animatableElements = document.querySelectorAll(ANIMATE_SELECTOR);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "משקים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '74a30ebe-380b-4623-a912-7cfbe8829839',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '74a30ebe-380b-4623-a912-7cfbe8829839';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '74a30ebe-380b-4623-a912-7cfbe8829839';
